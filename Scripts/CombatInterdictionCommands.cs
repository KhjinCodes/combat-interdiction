...
{
    public class CombatInterdictionCommands
    {
        private CombatInterdictionSettings settings = null;
        ...
        public const string COMMAND_PREFIX = "/c";

        public CombatInterdictionCommands()
        {
        ...
        {
            settings = CombatInterdictionSession.Instance.Settings;
        ...
        {
            // Extract the command
            string cleanMessage = message.Replace(COMMAND_PREFIX, string.Empty).Trim();
            ...
            {
                Utilities.NotifyMessage($"Remote message arrived:\n{command.Message}", "Green");
            }
        ...
        private void HandleGetAllSettings(CommandDetails command)
        {
            if (Utilities.IsServer())
            {
                string currentSettings = settings.GetCurrentSettings();
                SendResponse(currentSettings, command);
            }
            else
            {
                messaging.MessageServer(command.Message);
